...
# |                                   Units                                       |
# +===============================================================================+
from opensees.units.iks import inch, kip, sec, ft, lbf as lb, ksi
...
ops.section("Fiber",  colSecTag, shape=("W14X90", matTag, (20,4)))
ops.section("Fiber", beamSecTag, shape=("W18X76", matTag, (20,4)))
...
for s in range(0, rows):
    ...
    if s < 3:
        ...
        plt.plot(pushover_output["disp"], pushover_output["force"], "-k", 
                 linewidth=2.0, label="$U$")
        plt.plot(pushover_output["disp"], pushover_output["force"] +
                 pushover_output[f"sensLambda_{s+1}"] * ParamVars[s], "--k", 
                 linewidth=1.5, 
                 label=f"$U + (\partial V_b/\partial {ParamSym[s]}){ParamSym[s]}$")
        plt.plot(pushover_output["disp"], pushover_output["force"] -
                 pushover_output[f"sensLambda_{s+1}"] * ParamVars[s], "-.k", linewidth=1.5, 
                 label=f"$U - (\partial V_b/\partial {ParamSym[s]}){ParamSym[s]}$")
        plt.fill_between(pushover_output["disp"], 
                         pushover_output["force"] + pushover_output[f"sensLambda_{s+1}"] * ParamVars[s], 
                         pushover_output["force"] - pushover_output[f"sensLambda_{s+1}"] * ParamVars[s], color='grey', alpha=0.15)
